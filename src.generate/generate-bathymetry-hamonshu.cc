// Generate Natural Earth bathymetry with Izzi Hamonshu wave fields.
// -*- mode: C++ -*-
...
namespace cart0freak0::bathymetry_hamonshu_generation {
...
void
hamonshu_require(const bool condition, const std::string& message)
{
  if (!condition)
...
std::string
clip_path_markup(const depth_style& depth, std::string geometry)
{
  replace_all(geometry, R"(fill-rule="evenodd")",
              R"(fill-rule="evenodd" clip-rule="evenodd")");
  std::ostringstream output;
  output << "<clipPath id=\"" << catalogue::clip_id(depth)
...
{
  hamonshu_require(site_index < catalogue::field_voronoi_site_count,
                   "Hamonshu Voronoi site index is out of range");
                    ...
                    const generation::projection_context& context)
{
  ...
  hamonshu_require(result.total == grid.cell_count(),
                   "Hamonshu field has the wrong cell count");
                ...
                const generation::projection_context& context)
{
  ...
  std::ostringstream output;
  output << "<metadata id=\"bathymetry-hamonshu-metadata\""
         << " data-source=\"Natural Earth 5.1.1 1:10m physical vectors; "
            "Mori Yuzan Hamonshu volume 2 via Izzi\""
         << " data-projection=\"" << spec.argument << "\""
         << " data-depth-count=\"" << catalogue::depth_styles.size() << "\""
         << " data-field-mode=\"depth-hamonshu-voronoi-groups\""
         ...
         << " data-depth-controls=\"density curvature blue hue\""
         << " data-pattern-paint=\"stroke\""
         ...
         << ">Natural Earth depth thresholds contain explicit, overlapping "
            "Izzi Hamonshu wave linework selected in projected-page Voronoi "
            "groups; depth increases density, curvature, and blue hue."
            "</metadata>\n";
  return output.str();
}

svg::style
motif_style(const depth_style& depth, const std::size_t index)
{
  const double stroke_width
    = catalogue::field_stroke_width * (0.88 + 0.06 * (index % 4));
  return {
    svg::color::none, 0,
    depth.color, catalogue::field_graphic_opacity, stroke_width,
  };
}
        ...
        const generation::projection_context& context)
{
  ...
  key.start_element("bathymetry-hamonshu-key");
  key.add_title("Representative depth to Hamonshu wave-form key");
  ...
  background.add_style(
    {catalogue::ground_color, 0.94, svg::color_qi {23, 63, 72}, 0.72, 0.012});
  background.add_raw("id=\"bathymetry-hamonshu-key-background\" rx=\"0.12\"");
  background.finish_element();
  key.add_element(background);
  key.add_raw(key_text_markup(
    left + margin, top + 0.28, "DEPTH / HAMONSHU / ORIGINAL BLUE RAMP"));

  for (std::size_t index = 0; index != catalogue::depth_styles.size(); ++index)
    {
      const depth_style& depth = catalogue::depth_styles[index];
      const catalogue::field_variation& variation
        = catalogue::field_variations[index];
      ...
      const double factor
        = motif_diameter
          / (catalogue::field_base_diameter * variation.box_factor);
      const std::string path = catalogue::make_field_motif_path(
        depth, origin, variation, factor);
      key.add_element(svg::make_path(
        path, motif_style(depth, index),
        "bathymetry-hamonshu-key-motif-" + std::to_string(index + 1),
        true, R"(stroke-linecap="round" stroke-linejoin="round")"));
      const std::string label = catalogue::depth_text(depth.depth_metres)
        + "  density=" + catalogue::ratio_text(depth.density)
        + "  curve=" + catalogue::ratio_text(depth.curvature);
      key.add_raw(key_text_markup(
...
{ return generation::output_basename("bathymetry-hamonshu", spec); }
...
generate(const generation::projection_spec& spec)
{
  ...
  generation::projection_document document(
    basename, "Natural Earth 1:10m bathymetry rendered with progressively "
              "denser blue-hued Izzi Hamonshu wave fields in the "
  ...
  for (std::size_t index = 0; index != catalogue::depth_styles.size(); ++index)
    {
      ...
      hamonshu_require(depth.layer_id == source.id,
                       "Hamonshu catalogue and Natural Earth depths differ");
      hamonshu_require(color_text(depth.color)
                         == color_text(source.style._M_fill_color),
                       "Hamonshu catalogue and Natural Earth blue ramp differ");
      ...
      hamonshu_require(!geometry.empty(),
                       "bathymetry Hamonshu clip geometry is empty");
      definitions.add_raw(clip_path_markup(depth, geometry.str()));
      layer_stats.push_back(stats);
      std::cout << source.id << ": " << stats.source_features
                << " features, " << stats.paths << " clip paths, "
                << stats.points << " points, "
                << catalogue::depth_title(depth) << '\n';
    }
  definitions.finish_element();
  document.add_element(definitions);

  svg::group_element bathymetry;
  bathymetry.start_element("bathymetry-hamonshu");
  bathymetry.add_title(
    "Twelve nested depth thresholds with blue, Voronoi-grouped Hamonshu "
    "wave fields");
  for (std::size_t depth_index = 0;
       depth_index != catalogue::depth_styles.size(); ++depth_index)
    {
      const depth_style& depth = catalogue::depth_styles[depth_index];
      const hamonshu_field field = make_hamonshu_field(
        depth, depth_index, context);
      svg::group_element layer;
      layer.start_element(std::string(depth.layer_id));
      layer.add_title(catalogue::depth_title(depth));
      ...
      ground.add_style({catalogue::ground_color, 1,
                        svg::color::none, 0, 0});
      ground.add_raw(
        "id=\"" + std::string(depth.layer_id)
          + "-hamonshu-ground\" clip-path=\"url(#"
          + catalogue::clip_id(depth) + ")\"");
      ...
      for (std::size_t variation_index = 0;
           ...
           ++variation_index)
        {
          const catalogue::field_variation& variation
            = catalogue::field_variations[variation_index];
          const hamonshu::pattern_spec& pattern
            = catalogue::pattern(variation);
          const std::string id
            = catalogue::field_variation_id(depth, variation_index);
          svg::group_element variation_group;
          variation_group.start_element(id);
          variation_group.add_title(
            catalogue::field_variation_title(depth, variation_index));
          const std::string attributes
            = "clip-path=\"url(#" + catalogue::clip_id(depth)
              + ")\" stroke-linecap=\"round\" stroke-linejoin=\"round\""
                " data-instance-count=\""
              + std::to_string(field.counts[variation_index])
              + "\" data-depth-density=\""
              + catalogue::ratio_text(depth.density)
              + "\" data-depth-curvature=\""
              + catalogue::ratio_text(depth.curvature)
              + "\" data-pattern-source=\""
              + hamonshu::pattern_id(pattern)
              + "\" data-box-factor=\""
              + catalogue::ratio_text(variation.box_factor)
              + "\" data-phase-turns=\""
              + catalogue::ratio_text(variation.phase_turns)
              + "\" data-rotation-turns=\""
              + catalogue::ratio_text(variation.rotation_turns)
              + "\" data-reflected=\""
              + (variation.reflected ? "true" : "false")
              + "\" data-offset-x-cells=\""
          ...
          variation_group.add_element(svg::make_path(
            ...
            motif_style(depth, variation_index), id + "-forms", true,
            attributes));
      ...
      std::cout << depth.layer_id << ": " << field.total
                << " explicit Hamonshu field cells across "
                << catalogue::field_voronoi_site_count
                << " Voronoi regions and "
                << catalogue::field_variations.size() << " motifs\n";
    }
...
verify(const std::string& generated,
       const generation::projection_context& context)
{
  hamonshu_require(generated.find(generation::view_box_fragment(context))
                     != std::string::npos,
                   "generated bathymetry Hamonshu SVG has the wrong viewBox");
  hamonshu_require(
    generated.find("id=\"bathymetry-hamonshu-metadata\"")
        != std::string::npos
      && generated.find(
           "data-field-mode=\"depth-hamonshu-voronoi-groups\"")
           != std::string::npos
      && generated.find("data-field-voronoi-site-count=\"24\"")
           != std::string::npos
      && generated.find("data-graphic-opacity=\"0.3\"")
           != std::string::npos,
    "generated bathymetry Hamonshu SVG lacks field metadata");
  const std::size_t cells = make_field_grid(context).cell_count();
  hamonshu_require(
    generated.find("data-field-cell-count-per-depth=\""
      ...
      && generated.find("data-field-curve-instance-count=\""
                        + std::to_string(
                          cells * catalogue::depth_styles.size()) + "\"")
           != std::string::npos,
    "generated bathymetry Hamonshu SVG has incorrect field counts");
  hamonshu_require(
    generated.find("<g id=\"bathymetry-hamonshu\">")
        != std::string::npos
      && generated.find("<g id=\"bathymetry-hamonshu-key\">")
           != std::string::npos,
    "generated bathymetry Hamonshu SVG lacks semantic groups");
  hamonshu_require(
    token_count(generated, "<clipPath id=\"bathymetry-hamonshu-clip-")
          == catalogue::depth_styles.size()
      && token_count(generated, "-hamonshu-variation-")
           == 2 * catalogue::depth_styles.size()
                * catalogue::field_variations.size()
      && token_count(generated, "data-pattern-source=\"")
           == catalogue::depth_styles.size()
                * catalogue::field_variations.size()
    ...
    "generated bathymetry Hamonshu SVG has incomplete definitions");

  std::size_t previous_position = 0;
  for (const depth_style& depth : catalogue::depth_styles)
    {
      const std::string group = "<g id=\"" + std::string(depth.layer_id)
                                + "\">";
      const std::size_t position = generated.find(group);
      hamonshu_require(position != std::string::npos
                         && position >= previous_position,
                       "bathymetry Hamonshu depth groups are missing or "
                       "out of paint order");
      previous_position = position;
      hamonshu_require(
        generated.find("id=\"" + std::string(depth.layer_id)
                       + "-hamonshu-ground\"") != std::string::npos
          && generated.find("clip-path=\"url(#" + catalogue::clip_id(depth)
                            + ")\"") != std::string::npos,
        "bathymetry Hamonshu depth lacks its ground and clip");
      for (std::size_t variation_index = 0;
           variation_index != catalogue::field_variations.size();
           ++variation_index)
        {
          const std::string id
            = catalogue::field_variation_id(depth, variation_index);
          hamonshu_require(
            generated.find("<g id=\"" + id + "\">") != std::string::npos
              && generated.find("id=\"" + id + "-forms\"")
                   != std::string::npos,
            "bathymetry Hamonshu field variation is missing");
        }
    }

  hamonshu_require(token_count(generated, "clip-rule=\"evenodd\"") > 100,
                   "bathymetry Hamonshu clips do not preserve polygon holes");
  hamonshu_require(generated.find(" nan") == std::string::npos
                     && generated.find(" -nan") == std::string::npos
                     && generated.find(" inf") == std::string::npos
                     && generated.find(" -inf") == std::string::npos,
                   "generated bathymetry Hamonshu SVG has non-finite data");
  generation::verify_configured_label_font(
    generated, "Bathymetry Hamonshu SVG");
  ...
  for (const natural_earth::layer_spec& source
    ...
    hamonshu_require(
      generated.find(color_text(source.style._M_fill_color))
        != std::string::npos,
      "generated bathymetry Hamonshu SVG omits a Natural Earth depth blue");
}
...
main(const int argc, char** argv)
{
  try
    {
      return cart0freak0::bathymetry_hamonshu_generation::run(argc, argv);
    }
  catch (const std::exception& error)
    {
      std::cerr << "generate-bathymetry-hamonshu: " << error.what() << '\n';
      return 1;
    }
}
