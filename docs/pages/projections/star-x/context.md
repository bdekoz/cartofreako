...
The checked API 3 runtime implements both components of the current Star-X
forward and reverse transformation. Carrier component `0` reverses the Star-X
group transform before applying the checked Cahill–Keyes inverse. Unified-cap
component `1` recovers bearing around its registered South Pole and solves the
bounded source-radius equation on `[-90°,-60°]`. A result retains a component
identifier because the ordinary carrier and the top-painted cap can occupy
overlapping page regions. The [forward/reverse API](../../runtime/projection-api.md#star-x)
defines candidate, cutoff, quadrant-seam, overlap, and South-Pole behavior.
