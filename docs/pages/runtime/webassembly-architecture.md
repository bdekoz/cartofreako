...
**Status:** geometry ABI 1 complete; runtime API 3 all-family reverse
extension completed 2026-08-10;
measured WebGL/LOD remains optional
...
[Originating evaluation](../history/generation-status-20260806.md)
...
- runtime API 3 structured forward results plus candidate-aware reverse for
  every family, including separate Star-X carrier and Antarctic-cap
  components;
...
flowchart LR
  ...
  POINT["API 3 forward / reverse candidates"]
...
carrier. They should not assume a globally unique inverse: API 3 exposes
face/component-qualified candidates for every family. MapLibre integration remains a custom WebGL
...
[originating evaluation](../history/generation-status-20260806.md#context-existing-browser-cartography).
...
| Native forward/reverse contract | `tests/test-forward-reverse-projection-api.cc` checks 1,560 Cahill–Keyes zone samples plus seams/poles, all 24 AuthaGraph sectors and all 23 Dymaxion faces plus global lattices, all 5,120 faces in all six Myriahedral layouts, all Voronoi faces, Star-X carrier/cap/cut/pole semantics, batches, boundaries, and outside points |
...
## Deliberate API 3 / ABI 1 boundaries

- No false global inverse. Every family returns face/component-qualified
  candidates and preserves cut, periodic, singular, and Star-X overlap
  ambiguity. Use feature IDs plus a planar index for feature picking.
...
5. Evaluate TopoJSON/shared-arc input for stable feature IDs and smaller base
   map transfers.
6. Profile API 3 inverse throughput with and without native-cell/component
   qualification before choosing game-engine batch-size defaults.
