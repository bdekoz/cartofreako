...
Both release targets require the deterministic gate chain first:

```sh
make all-experiments-resilient
```

The gate runs, in order: `make clean` (implied by the
`all-experiments-fetch` prerequisite), every external fetch, the resilient
full-asset build (`all-resilient`), and every non-release experiment.
`release-github` and `release-ucb-aao-s3` both list
`all-experiments-resilient` as a prerequisite, so invoking either release
target re-runs the entire gate chain — a release is always a full rebuild by
design.

The recommended release sequence is therefore:

```sh
make all-experiments-resilient
git status --porcelain
git add <any tracked diffs> && git commit -m '...'
git tag <release-tag> HEAD
make release-github \
  GITHUB_RELEASE_TAG=<release-tag> \
  GITHUB_RELEASE_TITLE='...' \
  GITHUB_RELEASE_NOTES=docs/pages/releases/<release-notes>.md
```

Because the gate re-runs when `release-github` is invoked, the recipe's
clean-worktree and tag-points-at-HEAD checks remain meaningful: any tracked
evidence regenerated by the gate must already be committed before the release
recipe verifies the tree.
