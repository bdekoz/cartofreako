...
requireCondition(release.artifactCount === 205 && release.passCount === 31
    && release.layoutCount === 11 && release.sliceCount === 14,
'consumer layout corpus counts changed');
requireCondition(release.indexes.length === 43,
    'consumer layout must have one primary, 31 pass, and 11 layout indexes');
...
requireCondition(artifactIndex.artifactCount === 205
    && artifactIndex.artifacts.length === 205
    && new Set(artifactIndex.artifacts.map(value => value.id)).size === 205,
...
console.log('Stage 15I consumer layout passed: local-only candidate, 205 artifacts, 43 indexes, versioned WASM pair, no completion marker, no promoted 2K controls.');
