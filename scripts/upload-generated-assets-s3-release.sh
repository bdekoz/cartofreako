...
readonly default_prefix=cartofreako/v12
...
Validate, dry-run, and optionally publish the immutable Cartofreako v12 object
...
for command_name in cmp comm curl diff find gzip jq mktemp rclone \
...
for required_path in README.md viewer.html SHA256SUMS release.json \
  package/assets.generated.v12.tar.xz tree/svg tree/pdf tree/png \
  tree/thumbnail/cahill-keyes; do
...
find "$data_root/tree/svg" -maxdepth 1 -type f -name '*.svg.gz' -print0 | \
  xargs -0 -r -n 8 -P 4 gzip --test --

tree_file_count=$(find "$data_root/tree" -type f | wc -l)
raw_svg_count=$(find "$data_root/tree/svg" -maxdepth 1 -type f \
  -name '*.svg' | wc -l)
svg_gzip_count=$(find "$data_root/tree/svg" -maxdepth 1 -type f \
  -name '*.svg.gz' | wc -l)
pdf_count=$(find "$data_root/tree/pdf" -maxdepth 1 -type f -name '*.pdf' | wc -l)
png_count=$(find "$data_root/tree/png" -maxdepth 1 -type f -name '*.png' | wc -l)
thumbnail_count=$(find "$data_root/tree/thumbnail/cahill-keyes" -maxdepth 1 \
  -type f -name '*.png' | wc -l)
if [[ $tree_file_count -ne 589 || $raw_svg_count -ne 0 ||
      $svg_gzip_count -ne 187 || $pdf_count -ne 187 ||
      $png_count -ne 187 || $thumbnail_count -ne 28 ]]; then
...
general_upload_count=$(find "$data_root" -type f ! -path "$data_root/tree/svg/*" \
  ! -name release.json | wc -l)
general_upload_bytes=$(find "$data_root" -type f ! -path "$data_root/tree/svg/*" \
  ! -name release.json -printf '%s\n' | \
  awk '{ total += $1 } END { printf "%.0f", total }')
svg_gzip_bytes=$(find "$data_root/tree/svg" -maxdepth 1 -type f \
  -name '*.svg.gz' -printf '%s\n' | \
  awk '{ total += $1 } END { printf "%.0f", total }')
if [[ $payload_count -ne 592 || $release_count -ne 594 ||
      $general_upload_count -ne 406 ]]; then
...
package_sha256=$(sha256sum "$data_root/package/assets.generated.v12.tar.xz" | \
  awk '{print $1}')
...
if ! jq -e \
  ...
  '.complete == true and
   ...
   .inventory.published_tree_files == 589 and
...
readonly obsolete_v2_pilot=tree/svg/geometry-ck-44-22.svg
obsolete_v2_pilot_present=false
if [[ -s $unexpected_remote_paths ]]; then
  if [[ $prefix_complete != true &&
        $(wc -l < "$unexpected_remote_paths") -eq 1 &&
        $(sed -n '1p' "$unexpected_remote_paths") == "$obsolete_v2_pilot" ]]; then
    obsolete_v2_pilot_present=true
    printf '[notice] Found the one obsolete SVG pilot from the failed uploader-2 attempt.\n'
  else
...
if [[ $obsolete_v2_pilot_present == true ]]; then
  readonly obsolete_v2_pilot_label=$bucket/$prefix/$obsolete_v2_pilot
  printf 'The failed uploader-2 attempt left this obsolete object:\n  %s\n' \
    "$obsolete_v2_pilot_label" >/dev/tty
  printf 'Type that exact object path to remove only its current version:\n> ' \
    >/dev/tty
  IFS= read -r cleanup_confirmation </dev/tty
  if [[ $cleanup_confirmation != "$obsolete_v2_pilot_label" ]]; then
    printf 'Obsolete-pilot confirmation did not match; upload cancelled.\n' >&2
    exit 2
  fi
  rclone deletefile "$remote_path$obsolete_v2_pilot" \
    "${rclone_common[@]}" "${rclone_probe[@]}"
  rclone lsf "$remote_path" --recursive --files-only \
    "${rclone_common[@]}" --fast-list | LC_ALL=C sort > "$remote_paths"
  if grep -Fqx "$obsolete_v2_pilot" "$remote_paths"; then
    printf 'The obsolete pilot is still present; upload stopped.\n' >&2
    exit 1
  fi
  printf '[ok] Removed only the obsolete incomplete-release pilot.\n'
fi

svg_pilot=tree/svg/geometry-ck-44-22.svg.gz
png_pilot=tree/thumbnail/cahill-keyes/geometry-ck-44-22.png
printf '[stage 1/9] Uploading two browser-facing pilot objects...\n'
rclone copy "$data_root" "$remote_path" \
  "${rclone_common[@]}" "${rclone_progress[@]}" "${copy_safety[@]}" \
  --include "/$svg_pilot" \
  --header-upload 'Content-Type: application/gzip' \
...
printf '[stage 2/9] Verifying anonymous browser delivery of both pilots...\n'
...
printf '[stage 3/9] Uploading %s package, document, viewer, PDF, and PNG objects (%s)...\n' \
  "$general_upload_count" "$(format_bytes "$general_upload_bytes")"
rclone copy "$data_root" "$remote_path" \
  "${rclone_common[@]}" "${rclone_progress[@]}" "${copy_safety[@]}" \
...
printf '[stage 4/9] Verifying anonymous same-origin viewer delivery...\n'
if ! curl --connect-timeout 8 --max-time 60 --fail --silent --show-error \
...
printf '[stage 5/9] Uploading %s explicit SVG gzip objects (%s)...\n' \
  "$svg_gzip_count" "$(format_bytes "$svg_gzip_bytes")"
rclone copy "$data_root" "$remote_path" \
  "${rclone_common[@]}" "${rclone_progress[@]}" "${copy_safety[@]}" \
...
printf '[stage 6/9] Verifying every payload object before completion...\n'
...
printf '[stage 7/9] Publishing release.json last...\n'
rclone copy "$data_root" "$remote_path" \
  "${rclone_common[@]}" "${rclone_progress[@]}" "${copy_safety[@]}" \
...
printf '[stage 8/9] Running final release and public-marker verification...\n'
rclone check "$data_root" "$remote_path" \
