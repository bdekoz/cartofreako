# Cumulative network-swarm snapshot

This directory pins the offline input and rendering profile for the
`generate-network-swarm` pass. The archive was copied without modification
from
...
uncompressed digests are recorded in `network-swarm-profile.json`;
`SHA256SUMS`
...
the Makefile's `NETWORK_SWARM_SOURCE` override. ZIP input must contain one safe,
flat JSON member, pass its CRC check, and expand to no more than 64 MiB. The
prepared file lives under the ignored `assets.static/network-swarm/.prepared/`
